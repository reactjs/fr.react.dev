...
### Comment passer un gestionnaire d'événements (comme onClick) à un composant ? {#how-do-i-pass-an-event-handler-like-onclick-to-a-component}

On peut passer un gestionnaire d'événements et d'autres fonctions dans les props d'un composant :
...
Il y'a plusieurs façons de s'assurer que des fonctions ont accès aux attributs du composant comme `this.props` et `this.state`, qui dépendent de la syntaxe et de l’outillage que vous utilisez.
...
### Est-il acceptable d'utiliser une fonction fléchée dans la méthode `render` ? {#is-it-ok-to-use-arrow-functions-in-render-methods}
...
Avec React, vous n’avez généralement besoin de lier que les méthodes que vous *passez* à d'autres composants. Par exemple, `<button onClick={this.handleClick}>` passe `this.handleClick`, donc vous devez la lier. Cependant, il n'est pas nécessaire de lier la méthode `render` ou les méthodes de cycle de vie : on ne les passe pas à d'autres composants.
...
### Pourquoi ma fonction est appelée chaque fois que le composant s’affiche ? {#why-is-my-function-being-called-every-time-the-component-renders}
...
```jsx
render() {
  // Correct : handleClick est passé par référence !
  return <button onClick={this.handleClick}>Cliquez ici</button>
}
```
...
Jetez un coup d'œil à [cette visualisation](http://demo.nimius.net/debounce_throttle/) pour une comparaison des fonctions `throttle` et `debounce`.
...
Le _throttling_ évite qu'une fonction ne soit appelée plus d'une fois dans un certain laps de temps. L'exemple ci-dessous limite le gestionnaire d'événements de clic pour éviter qu'il ne soit appelé plus d'une fois par seconde.
...
class ScrollListener extends React.Component {
  ...
  handleScroll(e) {
    ...
    // Si on reçoit plusieurs mises à jour dans la même passe, on ne publiera que la dernière valeur.
