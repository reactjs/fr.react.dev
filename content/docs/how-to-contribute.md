...
Nous utilisons les [GitHub Issues](https://github.com/facebook/react/issues) pour les bugs publics. Nous les surveillons attentivement et essayons d'être transparents sur le développement en cours d’un correctif interne. Avant de créer une nouvelle demande, essayez de vérifier que votre problème n'a pas déjà été signalé.
...
Facebook a un [programme de récompenses](https://www.facebook.com/whitehat/) pour la communication sécurisée de problèmes de sécurité. En conséquence, merci de ne pas créer d'issue publique pour ça : suivez le processus expliqué sur la page du programme.
...
* `yarn linc` est comme `yarn lint` mais va plus vite car elle ne vérifie que les fichiers qui ont changé sur votre branche.
...
* `yarn debug-test` est exactement comme `yarn test` mais avec un débogueur. Ouvrez `chrome://inspect` et appuyez sur « Inspecter ».
* `yarn flow` lance les vérifications de types [Flow](https://flowtype.org/).
* `yarn build` crée un dossier `build` avec tous les modules.
* `yarn build react/index,react-dom/index --type=UMD` crée des builds UMD seulement de React et ReactDOM.
...
Ensuite, notre linter attrapera la plupart des problèmes qui pourraient exister dans votre code.
...
Vous voudrez peut-être regarder cette [cette courte vidéo](https://www.youtube.com/watch?v=wUpPsEcGsg8) (26 mins) (en anglais) qui présente comment contribuer à React.
...
Pour avoir une vue d'ensemble réaliste de l’expérience de première contribution à React, jetez un coup d’œil à [cette présentation divertissante de ReactNYC](https://www.youtube.com/watch?v=GWCcZ6fnpn4) (en anglais).
