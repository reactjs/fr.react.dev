...
Ceci dit, pendant le démontage du composant, les fonctions de *nettoyage* d’effet s’exécutaient de façon synchrone (comme pour la méthode synchrone `componentWillUnmount` sur les composants à base de classes). Nous avons pu constater que c’était loin d’être idéal pour les applis d’une certaine taille, car ça ralentissait les transitions entre écrans massifs (ex. bascule entre onglets).

**Dans React 17, la fonction de nettoyage d’effet s’exécutera toujours de façon asynchrone ; par exemple, si le composant est en cours de démontage, le nettoyage aura lieu _après_ que l’écran aura été mis à jour.**
...
Qui plus est, React 17 exécutera toujours toutes les fonctions de nettoyage (pour tous les composants) avant d’exécuter de nouveaux effets.  React 16 garantissait seulement cet ordre pour les effets au sein d’un même composant.
