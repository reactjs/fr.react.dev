...
Vous vous attendez alors sans doute à ce que la `<div>` ait un arrière-plan rouge, quelle que soit la valeur de `toggle`.  Et pourtant, en faisant alterner la valeur de `toggle` entre `true` et `false`, la couleur d’arrière-plan démarre à `red` puis alterne entre `transparent` et `blue`, [comme l’illustre cette démo](https://codesandbox.io/s/serene-dijkstra-dr0vev).
