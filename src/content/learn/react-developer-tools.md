...
Désormais, si vous visitez un site web **construit avec React**, vous pouvez apercevoir les onglets _Components_ et _Profiler_.
...
Ensuite, connectez votre site en ajoutant la balise `<script>` suivante au début de la balise `<head>` de votre site :
```html {3}
<html>
  <head>
...
> Essayez de relancer l'appli si les outils de développement ne se connectent pas au bout de quelques secondes. 
