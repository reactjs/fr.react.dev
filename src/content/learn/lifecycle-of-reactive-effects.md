...
- Un composant _se monte_ lorsqu'il est ajouté à l'écran.
...
**C'est une bonne façon de penser avec les composants, mais _pas_ avec les Effets**. Voyez plutôt chaque Effet indépendament du cycle de vie de vos composants. Un Effet décrit la façon de [synchroniser un système extérieur](/learn/synchronizing-with-effects) avec les props et état actuels. Au fur et à mesure que votre code change, la synchronisation sera plus ou moins fréquente.
...
La fonction de nettoyage renvoyée par votre Effet spéficié comment **stopper la synchronisation** :
...
### Pourquoi la synchronisation peut-elle se produire plus d'une fois {/*why-synchronization-may-need-to-happen-more-than-once*/}
...
Grâce à ça, vous êtes maintenant connecté au même salon que celui choisit par l'utilisateur sur l'interface. La catastrophe est évitée !

Chaque fois que votre composant sera re-rendu avec un `roomId` différent, votre Effet se resynchronisera. Disons par exemple que l'utilisateur change le `roomId` de `"travel"` à `"music"`. React **arrêtera une nouvelle fois de synchroniser** votre Effet en appelant la fonction de nettoyage (en se déconnectant du salon `"travel"`). Puis, il **recommencera à se synchroniser** en exécutant le code avec la nouvelle prop `roomId` (en se connectant au salon `"music"`).

Enfin, lorsque votre utilisateur change d'écran, `ChatRoom` se démonte. Il n'est plus utile de rester connecté. React **arrêtera de synchroniser** votre Effet une dernière fois et vous déconnectera du salon `"music"`.
...
La structure de ce code peut vous inciter à voir que ce qui s'est passé comme une séquence de périodes de temps ne se chevauchant pas :
...
Ça vous rappelera peut-être que vous ne vous souciez pas de savoir si un composant est en train de se monter ou de se mettre à jour quand vous écrivez la logique de rendu qui crée le JSX. Vous décrivez ce qui doit être à l'écran et React [se charge du reste](/learn/reacting-to-input-with-state).
...
Remarquez ces trois messages lorsque le composant se monte pour la première fois :
...
La raison principale pour laquelle un Effet se resychronisera est que certaines données qu'il utilise ont changé. Dans le bac à sable précédent, sélectionné un autre salon de discussion. Remarquez comment votre Effet se resynchronise quand `roomId` change.
...
3. C'est pourquoi vous l'avez spécifié dans les dépendances de votre Effet (donc il se resynchronise quand `roomId` change).

Chaque fois que votre composant se réaffiche, React regardera le tableau des dépendances que vous avez définit. Si l'une des valeurs de ce tableau est différente de la valeur lors du précédent rendu, alors React resynchronisera votre Effet.

Par exemple, si vous avez passé `["general"]` lors du rendu initial, puis au prochain rendu vous avez passé `["travel"]`, React comparera `"general"` et `"travel"`. Ce sont des valeurs différentes (comparées avec [`Object.is`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is)), donc React resynchronisera votre Effet. En revanche, si votre composant effectue un nouveau rendu mais que `roomId` n'a pas changé, alors votre Effet restera connecté au même salon.
...
Résistez à l'envie d'ajouter de la logique sans rapport avec votre Effet uniquement parce qu'elle doit être exécutée en même temps qu'un Effet déjà écrit. Disons par exemple que vous voulez envoyer des événements d'analytique quand un utilisateur visite un salon. Vous avez déjà un Effet qui dépend de `roomId`, vous pourriez être tenté d'y ajouter l'appel à l'analytique :
...
Simplement parce que `serverUrl` ne change jamais à la suite d'un nouveau rendu. Il reste le même, quelque soit le nombre de fois où le composant est rendu ou pourquoi il l'est. Puisque `serverUrl` ne change jamais, ça n'aurait aucun sens de le spécifier en tant que dépendance. Après tout, les dépendances n'ont d'importance que si elles changent avec le temps !

En revanche, `roomId` peut être différent lors d'un nouveau rendu. **Les props, état et autres valeurs déclarées au sein d'un composant sont _réactifs_ parce qu'ils sont calculés pendant un rendu et participe au flux de données de React**.
...
function ChatRoom({ roomId }) { // Les props changent au cours du temps
  ...
  useEffect(() => {
    const connection = createConnection(serverUrl, roomId); // Votre Effet lit les props et état
    connection.connect();
    return () => {
      connection.disconnect();
    };
  }, [roomId, serverUrl]); // Vous indiquez à React que cet Effet « dépend de » ces props et état
  // ...
}
```
...
Du point de vue du composant, le tableau de dépendances vide `[]` signifie que cet Effet se connecte au salon de discussion seulement quand le composant se monte, puis se déconnecte qu'une fois le composant démonté (gardez en mémoire que React voudra toujours [se resynchroniser une fois de plus](#how-react-verifies-that-your-effect-can-re-synchronize) en phase de développement pour tester votre logique).
...
Les props et état ne sont pas les seules valeurs réactives. Les valeurs que vous calculez à partir d'elles sont aussi réactives. Si vos props ou état changent, votre composant sera à nouveau rendu et les valeurs ainsi calculées changeront également. C'est pourquoi toutes les variables présentes dans le composant utilisées par l'Effet doivent apparaître dans la liste des dépendances.

Imaginez que l'utilisateur puisse choisir le serveur de discussion dans une liste déroulante, mais qu'il puisse aussi configurer un serveur par défaut dans les paramètres. Supposez que vous ayez déjà mis l'état des paramètres dans un [contexte](/learn/scaling-up-with-reducer-and-context) et que vous y lisiez ces `settings`. Vous calculez maintenant le `serverUrl` en fonction du serveur sélectionné depuis les props et le serveur par défaut :
...
**Toutes les variables au sein du composant (y compris les props, état et les variables dans le corps de votre composant) sont réactives. Toute valeur réactive pouvant changer lors d'un nouveau rendu, vous devez l'inclure dans les dépendances de votre Effet.**
...
Ça peut ressembler à une erreur React, mais en réalité React signale un bug dans votre code. `roomId` et `serverUrl` peuvent tous deux changer au cours du temps, mais vous oubliez de resynchroniser votre Effet lorsqu'ils changent. Vous resterez lié aux valeurs initiales de `roomId` et `serverUrl`, même si l'utilisateur choisit des valeurs différentes depuis l'interface.
...
Vous pouvez aussi les déplacer *à l'intérieur de l'Effet*. Ils ne sont pas calculés durant le rendu et ne sont donc pas réactifs :
...
**Vous ne pouvez pas « choisir » vos dépendances**. Vos dépendances doivent inclure chaque [valeur réactive](#all-variables-declared-in-the-component-body-are-reactive) que vous lisez dans l'Effet. C'est imposé par le *linter*. Ça peut parfois aboutir à des problèmes comme des boucles infinies et des resynchronisations trop fréquentes de votre Effet. Ne corrigez pas ces erreurs en supprimant le *linter* ! Voici ce qu'il faut faire à la place :

* **Vérifiez que votre Effet représente un processus de sychronisation indépendant**. Si votre Effet ne synchronise rien du tout, [il est peut-être inutile](/learn/you-might-not-need-an-effect). S'il synchronise plusieurs choses indépendantes, [divisez-le](#each-effect-represents-a-separate-synchronization-process).
...
Dans les [pages](/learn/separating-events-from-effects) [suivantes](/learn/removing-effect-dependencies), vous apprendrez comment corriger ce code sans enfreindre les règles. Ça vaut toujours la peine d'être corrigé !
...
- Le *linter* vérifie que toutes les valeurs réactives utilisées à l'intérieur de l'Effet sont spécifiées en tant que dépendances.
...
Cependant, il y a un souci. Lorsque vous tapez dans la boite de message en dessous, le composant `ChatRoom` se reconnecte *également* au salon (vous pouvez remarquer ça en vidant la console puis en tapant dans le champ de saisie). Corrigez ce problème pour que ça ne se reproduise plus.
...
Cet Effet n'avait pas du tout de tableau de dépendances, il se resynchronisait donc après chaque rendu. D'abord, ajoutez le tableau. Ensuite, assurez-vous que chaque valeur réactive utilisée par l'Effet est spécifiée dedans. Par exemple, `roomId` est réactif (puisque c'est une prop), donc il doit être inclus dans le tableau. Ça permet de s'assurer que la reconnexion se fait quand l'utilisateur change de salon. En revanche, `serverUrl` est défini à l'extérieur du composant. C'est pourquoi il n'a pas besoin d'être dans le tableau.
...
#### Trouvez le bug avec une valeur obsolète {/*investigate-a-stale-value-bug*/}
...
**Si vous ne supprimez jamais le *linter*, vous ne rencontrerez pas de problèmes avec des valeurs obsolètes**. Il existe différentes façons de résoudre ce bug, mais vous devez toujours commencer par réactiver le *linter*. Ensuite, changez le code pour corriger l'erreur du *linter*.
...
Cette solution fonctionne, mais n'est pas idéale. Si vous ajoutez `console.log('Nouvelle souscription')` dans l'Effet, vous noterez qu'il resouscrit après chaque nouveau rendu. La resouscription est rapide, mais il serait quand même préférable de ne pas le faire aussi souvent.

Une meilleure correction serait de déplacer la fonction `handleMove` *à l'intérieur* de l'Effet. Ainsi, `handleMove` ne serait plus une valeur réactive et votre Effet ne dépendrait plus d'une fonction. À la place, il devra dépendre de `canMove` que votre code lit désormais à l'intérieur de l'Effet. Ça correspond au comportement que vous vouliez, puisque votre Effet reste synchronisé avec la valeur de `canMove` :
