...
Vous souhaitez parfois que l'état de deux composants change toujours ensemble. Pour ça, supprimez leur l'état et déplacez-le vers leur parent commun le plus proche, puis passez-le *via* les props. C'est ce que l'on appelle *faire remonter l'état*, et c'est l'une des choses les plus communes que vous ferez en écrivant du code React.
...
Chaque composant `Panel` a une variable booléenne d'état `isActive` qui définit si son contenu est visible ou non.
...
<Diagram name="sharing_state_child" height={367} width={477} alt="Une diagramme montrant un arbre à trois composants, un parent appelé Accordion et deux enfants appelés Panel. Les deux composants Panel contiennent une valeur isActive valant false.">

Initialement, l'état `isActive` de chaque `Panel` est égal à `false`, ils doivent donc apparaître repliés.
...
1. **Supprimez** l'état des composants enfants.
2. **Passez** la valeur codée en dur depuis le parent commun.
3. **Ajoutez** l'état au parent commun et passez-le en-dessous avec les gestionnaires d'événements.

Ça permettra au composant `Accordion` de coordonner les deux `Panel` et ainsi n'en déplier qu'un seul à la fois.

### Étape 1 : supprimez l'état des composants enfants {/*step-1-remove-state-from-the-child-components*/}
...
Maintenant, le composant parent du `Panel` peut *contrôler* `isActive` en [le passant en tant que props](/learn/passing-props-to-a-component). À l'inverse, le composant `Panel` n'a *plus de contrôle* sur la valeur de `isActive` — c'est désormais entre les mains du composant parent !
...
Éditez les valeurs de `isActive` codées en dur dans le composant `Accordion` et voyez le résultat à l'écran.
...
Dans cet exemple, un seul panneau doit être actif à un instant donné. Ça signifie que le parent commun `Accordion` doit garder une trace pour savoir *quel* panneau est actif. Pour la variable d'état, il pourrait utiliser un nombre représentant l'index du `Panel` actif plutôt qu'un `boolean` : 

```js
const [activeIndex, setActiveIndex] = useState(0);
```

Quand `activeIndex` vaut `0`, le premier panneau est actif, s'il vaut `1`, alors c'est le second.

Le fait d'appuyer sur le bouton « Afficher » dans l'un ou l'autre des `Panel` doit modifier l'index actif dans `Accordion`. Un `Panel` ne peut définir l'état `activeIndex` directement puisqu'il est défini à l'intérieur d'`Accordion`. Ce dernier doit *explicitement autoriser* le composant `Panel` de changer son état en [lui transmettant un gestionnaire d'événement en tant que prop](/learn/responding-to-events#passing-event-handlers-as-props) :
...
Ainsi s'achève la partie pour faire remonter l'état ! Déplacer l'état dans le composant parent commun vous permet de coordonner les deux panneaux. L'utilisation d'un index actif à la place de deux indicateurs « est affiché » a garantit qu'un seul panneau est actif à la fois. Enfin, en transférant le gestionnaire d'état à l'enfant permet à ce dernier de changer l'état du parent.

<DiagramGroup>

<Diagram name="sharing_state_parent" height={385} width={487} alt="Un diagramme montrant un arbre à trois composants, un parent appelé Accordion et deux enfants appelés Panel. Accordion contient une valeur activeIndex à 0 qui se transforme en une valeur isActive à true passé au premier Panel et une autre à false passée au second Panel." >

Au départ, la valeur de `isActive` de l'`Accordion` vaut `0`, le premier `Panel` reçoit donc `isActive = true`.

</Diagram>

<Diagram name="sharing_state_parent_clicked" height={385} width={521} alt="Le même diagramme que le précédent, avec la valeur de activeIndex du parent Accordion en surbrillance, indiquant un clic et dont la valeur a été changée en 1. Le flux en direction des deux composants enfants Panel est également en surbrillance, la valeur isActive passée à chacun des enfant est définie à l'opposé : false pour le premier Panel et true pour le second." >
...
Il est courant d'appeler « non contrôlé » un composant ayant un état local. Par exemple, le composant `Panel` d'origine avec une variable d'état `isActive` est non contrôlé parce que son parent ne peut pas influencer le fait que le panneau est actif ou non.
...
Les composants non contrôlés sont plus simples d'utilisation au sein de leurs parents puisqu'ils requièrent moisn de configuration. Cependant, ils sont moins flexibles lorsque vous souhaitez les coordonner ensemble. Les composants contrôlés sont extrêmement flexibles, mais ils nécessitent que les composants parents les configurent complétement avec des props.

Dans la pratique, « contrôlé » et « non contrôlé » ne sont pas des termes techniques stricts — chaque composant dispose généralement d'un mélange entre état local et props. Cependant, c'est une façon pratique de parler de la façon dont les composants sont conçus et des capacités qu'ils offrent.

En écrivant un composant, réfléchissez quelles informations doivent être contrôlées (*via* les props) et lesquelles doivent être non contrôlées (*via* l'état). Cependant, vous pouvez toujours changer d'avis par la suite et remanier le composant.

</DeepDive>

## Une source de vérité unique pour chaque état {/*a-single-source-of-truth-for-each-state*/}

Dans une application React, de nombreux composants auront leur propre état. Ces derniers peuvent « vivre » à proximité des composants feuilles (les composants en bas de l'arbre) tels que les champs de saisie. D'autres peuvent « vivre » plus proche de la racine de l'appli. Ainsi, même les bibliothèques de routage côté client sont généralement implémentées en stockant la route actuelle dans un état React, puis la transmettent au travers des props !

**Pour chaque élément unique de l'état, vous devez choisir le composant à qui il va « appartenir ».** Ce principe est également connu comme [« une source de vérité unique »](https://en.wikipedia.org/wiki/Single_source_of_truth) (*lien en anglais, NdT*). Ça ne signifie pas que tout l'état se situe à un seul endroit mais plutôt que pour _chaque_ élément d'état, il y a un composant _spécifique_ qui détient cet élément. Au lieu de dupliquer un état partagé entre des composants, il faut le *faire remonter* jusqu'à leur parent commun, puis le *passer vers le bas* aux composants qui en ont besoin.

Votre appli évoluera au fur et à mesure que vous y travaillerez. Il est fréquent que vous déplaciez un état vers le bas ou vers le haut alors que vous êtes en train de déterminer où chaque élément de l'état doit « vivre ». Ça fait partie du process !
...
* Lorsque vous souhaitez coordonner deux composants entre eux, déplacez leur état vers leur parent commun.
...
Ces deux champs de saisie sont indépendants. Faites en sorte qu'ils soient synchronisés : modifier l'un des champs doit également mettre à jour l'autre champ avec le même texte, et vice et versa.
...
Dans cet exemple, la `SearchBar` dispose de son propre état `query` qui contrôle le champ de texte. Son composant parent `FilterableList` affiche une `List` d'éléments, mais ne tient pas compte de la recherche.

Utilisez la fonction `filterItems(foods, query)` pour filtrer la liste selon la requête de recherche. Pour tester vos modification, vérifiez que le fait de taper « s » dans le champ de filtre réduit la liste à « Sushi », « Shish kebab » et « Dim sum ».

Remarquez que `filterItems` est déjà implémentée et importée afin que vous n'ayez pas à l'écrire vous-même !

<Hint>

Vous devrez supprimer l'état `query` et le gestionnaire `handleChange` de la `SearchBar` et les déplacer dans `FilterableList`. Transmettez-les ensuite à `SearchBar` en tant que props `query` et `onChange`.
